...
class UNet_choh_skip(nn.Module):
	def __init__(self, in_channels=1, n_classes=1, depth=5, wf=6, padding=True, batch_norm=False, up_mode='upconv', n_hidden=16):
		...
		for i in reversed(range(depth - 1)):
		    ...
		    #
		# self.last = nn.Conv2d(prev_channels, n_classes, kernel_size=1)
		# N_HIDDEN_LRNN_2 = 16 #12 #8 #10 #12 #14 #16 #8 #16 #8
...
def main():
    ...
    # # choh_data_train = choh_Dataset() #FaceLandmarksDataset() choh_Dataset()
    # # choh_data_train = choh_Dataset_single() #FaceLandmarksDataset() choh_Dataset()
    # choh_data_train = choh_icsl_single() #FaceLandmarksDataset() choh_Dataset()
    ...
    #         img_disp = np.squeeze(img_batch[7,0,:,:])
